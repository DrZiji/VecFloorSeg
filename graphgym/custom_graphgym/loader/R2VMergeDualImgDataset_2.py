...
@register_loader('R2V_Merge_DUALIMG_2')
def load_dataset_R2V(format, name, dataset_dir):
    if format == 'PyG':
        ...
        if cfg.dataset.augmentation:
            ...
            aug = None
            img_aug = None
        dataset_train = R2VMergeDualIMGFloorplan2(
            rootDir=dataset_dir, split='train', augmentation=aug,
            img_transform=imgTransform, img_augmentation=img_aug
        )
        dataset_val = R2VMergeDualIMGFloorplan2(rootDir=dataset_dir, split='test', img_transform=imgTransform)
        dataset_test = R2VMergeDualIMGFloorplan2(rootDir=dataset_dir, split='test', img_transform=imgTransform)
...
@register_dataset('R2V_Merge_DUALIMG_2')
class R2VMergeDualIMGFloorplan2(InMemoryDataset):
    """
    处理fplanVertex和fplanVeno graph，并且建立两者的对应关系
    """
    CLASSES = (
        "wall",
        "kitchen",
    ...
    def __init__(
        ...
        super(R2VMergeDualIMGFloorplan2, self).__init__()

        MAPPING_SHAPE = 256

        imageFolder = os.path.join(rootDir, 'img_dir', split)
    ...
    def __getitem__(self, item):
        ...
        imgName = self.imgNames[item].split('.')[0]
    ...
    def _loadProcessedData(self):
        ...
        with open(os.path.join(
                self.rootDir, 'merge_' + self.partition + '_phase_V4.pkl'
        ), 'rb') as f:
...
if __name__ == "__main__":
    with open('/home/ybc2021/Datasets/R2V_CAD/train_phase_V1.pkl', 'rb') as f:
        data = pickle.load(f)
    print()
    # transform = Compose([
    #     ToUndirected(),
    #     NormalizeScale()
    # ])
    trainDataset = R2VMergeDualIMGFloorplan2(rootDir='/home/ybc2021/Datasets/R2V_CAD',
                                 transform=None)
    # LightningDataset(
