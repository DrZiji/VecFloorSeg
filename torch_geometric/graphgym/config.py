...
def set_cfg(cfg):
    ...
    # Size of input dimension, Size of out dimension, i.e., number of labels to be predicted
    cfg.share.dim_node_in = 1
    cfg.share.dim_node_out = 0

    # Size of input and output edge dimension
    cfg.share.dim_edge_in = 0
    cfg.share.dim_edge_out = 0

    # size of task output dimension
    cfg.share.dim_out = 0
    cfg.share.aux_dim_out = 0

    # Number of dataset splits: train/val/test
    cfg.share.num_splits = 1

    cfg.share.imgEncoder_dim_out = -1
    # ----------------------------------------------------------------------- #
    # Dataset options
    # ----------------------------------------------------------------------- #
    ...
    # classification_multi, semantic_segmentation
    ...
    cfg.dataset.category = list()

    cfg.dataset.augmentation = True
    ...
    cfg.train.exist_root_dir = True

    # previous training should be removed.
    cfg.train.retrain = False
    ...
    cfg.model.has_aux = False

    cfg.model.aux_loss_fun = ''

    cfg.model.has_multi_stage = False

    cfg.model.multi_stage_loss_fun = ''

    cfg.model.loss_regularization = False
    ...
    cfg.gnn.imgEncoder = 'resnet'

    # ----------------------------------------------------------------------- #
    # pre Layer options
    # ----------------------------------------------------------------------- #
    cfg.gnn.pre_layer_cfg = CN(new_allowed=True)

    # ----------------------------------------------------------------------- #
    # Layer options
    # ----------------------------------------------------------------------- #
    cfg.gnn.layer_cfg = CN(new_allowed=True)

    # ----------------------------------------------------------------------- #
    # head options
    # ----------------------------------------------------------------------- #
    cfg.gnn.head_cfg = CN(new_allowed=True)
...
def set_run_dir(out_dir):
    ...
    # Make output directory
    if cfg.train.retrain:
        makedirs_rm_exist(cfg.run_dir)
    elif not os.path.exists(cfg.run_dir):
        os.makedirs(cfg.run_dir, exist_ok=True)
    else:
        logging.info('{} already exists.'.format(cfg.run_dir))
...
def from_config(func):
    ...
    def wrapper(*args, cfg: Any = None, **kwargs):
        # args是通过程序给func指定的参数，在cfg中找到的是剩余的参数，
        # arg_names是func需要的全部参数，所以要把args先去掉
        if cfg is not None:
